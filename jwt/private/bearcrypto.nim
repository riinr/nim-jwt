...
proc vtable(algorithm: Algorithm): auto =
    case algorithm.size:
    of S256:
        return addr sha256Vtable
    of S384:
        return addr sha384Vtable
    of S512:
        return addr sha512Vtable


proc oid(algorithm: Algorithm): auto =
    case algorithm.size:
    of S256:
        return HASH_OID_SHA256
    of S384:
        return HASH_OID_SHA384
    of S512:
        return HASH_OID_SHA512


proc hashLen(algorithm: Algorithm): auto =
    case algorithm.size:
    of S256:
        return sha256SIZE
    of S384:
...
proc signString*(algorithm: Algorithm, toSign: string, secret: string): seq[byte] =
  case algorithm.kind
  of HS:
    bearHMAC(algorithm.vtable, secret, toSign)
  of RS:
    bearSignRSPem(toSign, secret, algorithm.vtable, algorithm.oid, algorithm.hashLen)
  of ES:
    bearSignECPem(toSign, secret, algorithm.vtable, addr ecAllM15)
...
proc verifySignature*(algorithm: Algorithm, data: string, signature: seq[byte], secret: string): bool =
  ...
  of RS:
    bearVerifyRSPem(data, secret, signature, algorithm.vtable, algorithm.oid, algorithm.hashLen)
  of ES:
    bearVerifyECPem(data, secret, signature, algorithm.vtable, addr ecPrimeI15, algorithm.hashLen)